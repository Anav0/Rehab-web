...
export function filterTimeBlocksByDates(timeBlocks: TimeBlock[], unavailableDates: any): TimeBlock[] {
    ...
    for (let dateRange of unavailableDates) {
        ...
        timeBlocks = timeBlocks.filter(x => x.StartDate.getTime() < start.getTime() || x.StartDate.getTime() > end.getTime());
    }
    return timeBlocks
}