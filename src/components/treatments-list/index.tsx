...
interface TreatmentsListProps {
  treatmentsConstraints: Map<string, RawConstraint[]>;
}
...
export const TreatmentsList = (props: TreatmentsListProps) => {
  ...
  const constraintUI: {
    ...
    proximity: (data: RawConstraint) => {
      ...
      if (!constraintTreatment)
        throw new Error(
          `Nie znaleziono procedury o id ${proximity.TreatmentId}`
        );
      return (
        <span
          key={data.Type + "-" + constraintTreatment.Name + proximity.Offset}
        >
          {Math.abs(proximity.Offset)}min {beforeOrAfter}{" "}
          {constraintTreatment.Name}
        </span>
      );
    },
  };
  ...
  useEffect(() => {
    ...
    for (let treatment of treatments) {
      let treatmentConstraints = props.treatmentsConstraints.get
        ? props.treatmentsConstraints.get(treatment.Id)
        : [];
  ...
  return (
    <>
      ...
      <Modal
        title="Szczegóły dotyczące procedur"
